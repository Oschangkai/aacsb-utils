  ...
  public async Run(): Promise<void> {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        let q = query + `('${uuidv4()}', 'Presentation', '${teacher.Id}', N'${data[i].journals_title}', ${data[i].other_authors == '' ? null : "N'" + data[i].other_authors + "'"}, ${data[i].order_authors == '' ? null : "N'" + data[i].order_authors + "'"}, ${data[i].address_authors == '是' ? 1 : data[i].address_authors == '' ? null : 0}, ${data[i].publication == '是' ? 1 : data[i].publication == '' ? null : 0}, ${data[i].seminar == '' ? null : "N'" + data[i].seminar + "'"}, ${data[i].year_id_s == "" ? null : data[i].year_id_s}, ${data[i].month_id_s == "" ? null : data[i].month_id_s}, ${data[i].day_id_s == "" ? null : data[i].day_id_s}, ${data[i].year_id_e == "" ? null : data[i].year_id_e}, ${data[i].month_id_e == "" ? null : data[i].month_id_e}, ${data[i].day_id_e == "" ? null : data[i].day_id_e}, ${data[i].country == '' ? null : "N'" + data[i].country + "'"}, ${data[i].city == '' ? null : "N'" + data[i].city + "'"}, ${data[i].page_s == '' ? null : data[i].page_s}, ${data[i].page_e == '' ? null : data[i].page_e}, ${data[i].project == '' ? null : "N'" + data[i].project + "'"}, '${NIL_UUID}', GETDATE(), '${NIL_UUID}', GETDATE())`;
        // console.log(q);
